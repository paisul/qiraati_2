...
class Catatan_pelanggaran_M extends CI_Model
{
  ...
  // Total poin pelanggaran SEMUA kategori (Ibadah + Bahasa digabung) dalam rentang tanggal -
  // dipakai rating bintang "Adab Akhlak" otomatis di Rapor (keputusan user: semua kategori digabung).
  public function getTotalPoinRentang($id_siswa, $tgl_mulai, $tgl_selesai)
  {
    $this->db->select_sum('Points');
    $this->db->from('pelanggaran');
    $this->db->where('IdSiswa', $id_siswa);
    $this->db->where('Tgl >=', $tgl_mulai);
    $this->db->where('Tgl <=', $tgl_selesai);
