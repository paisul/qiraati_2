...
class Raport_M extends CI_Model
{
  ...
  // Rentang tanggal (MIN TglMulai s.d. MAX TglSelesai) target untuk kelas+periode+ajaran+semester
  // periode ujian ini - jadi acuan rentang tanggal untuk semua perhitungan rating bintang otomatis
  // (Kehadiran, Hafalan Doa & Qosidah, Adab Akhlak) di Raport::aksi_tampil(). NULL kalau belum ada
  // Target sama sekali untuk kelas & periode itu (menu Tahfidz > Target).
  public function getRentangTanggalPeriodeUjian($id_periode_ujian)
  {
    $query = 'SELECT MIN(`target`.`TglMulai`) AS TglMulai, MAX(`target`.`TglSelesai`) AS TglSelesai
    FROM `periodeujian`
    JOIN `target` ON `target`.`IdKelas`=`periodeujian`.`IdKelas`
      AND `target`.`IdPeriode`=`periodeujian`.`IdPeriode`
      AND `target`.`IdAjaran`=`periodeujian`.`IdAjaran`
      AND `target`.`IdSemester`=`periodeujian`.`IdSemester`
    WHERE `periodeujian`.`IdPeriodeUjian`="' . $id_periode_ujian . '"';
    $row = $this->db->query($query)->row_array();

    if (!$row || !$row['TglMulai'] || !$row['TglSelesai']) {
      return null;
    }
  ...
  public function getInfoPeriodeUjian($id_periode_ujian)
  {
    return $this->db->get_where('periodeujian', ['IdPeriodeUjian' => $id_periode_ujian])->row_array();
  }

  // Dipanggil kalau getRentangTanggalPeriodeUjian() di atas kosong - Target dan Periode Ujian
  // dibuat lewat form TERPISAH yang masing-masing bebas memilih Kelas/Periode/Ajaran/Semester
  // sendiri-sendiri, tanpa validasi silang. Jadi kalau match 4 kolomnya gagal, kemungkinan besar
  // BUKAN karena Target-nya belum ada sama sekali, tapi Tahun Ajaran/Semester yang dipilih saat
  // membuat Target beda dengan yang dipilih saat membuat Periode Ujian untuk Kelas+Periode yang
  // sama - method ini cari tahu Ajaran/Semester APA yang justru punya Target untuk Kelas+Periode
  // itu, supaya pesan errornya bisa menunjuk ke akar masalah yang sesungguhnya.
  public function getAjaranSemesterTargetUntukKelasPeriode($id_kelas, $id_periode)
  {
    // "DISTINCT" harus lewat distinct(), bukan ditaruh di dalam string select() - CI3 salah
    // mem-parsing identifier-nya kalau digabung jadi satu string ("DISTINCT ajaran.ThAjaran"),
    // menghasilkan SQL rusak (syntax error) - inilah penyebab pasti error 500 di Raport preview.
    $this->db->distinct();
    $this->db->select('ajaran.ThAjaran, semester.Semester');
    $this->db->from('target');
    $this->db->join('ajaran', 'ajaran.IdAjaran = target.IdAjaran', 'left');
    $this->db->join('semester', 'semester.IdSemester = target.IdSemester', 'left');
    $this->db->where('target.IdKelas', $id_kelas);
    $this->db->where('target.IdPeriode', $id_periode);
